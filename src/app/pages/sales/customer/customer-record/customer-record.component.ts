...
export class CustomerRecordComponent implements OnInit {
  ...
  columns:any[] = [
    { value: "customerId", viewValue: "ID", visible:true },
    { value: "customerCode", viewValue: "Code", visible:true, criteria: true },
    { value: "name", viewValue: "Name", visible:true, criteria: true },
    { value: "defaultSalesTypeCode", viewValue: "Sales Type", visible:true, criteria: true },
    { value: "tin", viewValue: "TIN", visible:true, criteria: true },
    { value: "statusCode", viewValue: "Status", visible:true }
  ];

  constructor(private _routeChangeService:RouteChangeService, private _customerService: CustomerService, private _dataTableService: DataTableService) { }

  ngOnInit(): void {

    /** Set the breadcrumb value expected output: Customer / Record. */
    ...
    this._customerService.getPaginated().pipe(      
      map((result) => { 
  ...
  pagingEvent(event:any) {
    ...
    this._customerService.getPaginated(event).pipe(      
      map((result) => { 
        this._dataTableService.updateDataTable(result.body);
        this._dataTableService.updateDataPaging(JSON.parse(result.headers.get('paging')!));
      })
  ...
  filterRecord(filterValue:any) {
    ...
    this._customerService.getPaginated(this.filterParameter).pipe(      
      map((result) => { 
        this._dataTableService.updateDataTable(result.body);
        this._dataTableService.updateDataPaging(JSON.parse(result.headers.get('paging')!));
      })
